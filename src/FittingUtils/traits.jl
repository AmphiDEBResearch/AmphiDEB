...
function metamorphosis_energy_budgets(sim::AbstractDataFrame)
    ...
    #@assert nrow(sim_tj1)==1 "Expected 1 row per DataFrame - missing grouping variable?"
    #@assert nrow(sim_tj2)==1 "Expected 1 row per DataFrame - missing grouping variable?"
...
function age_at_birth(sim::AbstractDataFrame; rule = "X_emb")
    ...
    if rule == "X_emb" 
        at_birth = sim[sim.X_emb .== minimum(sim.X_emb),:]
        @assert isapprox(at_birth.X_emb[1], 0, atol = 1e-3) 
        return at_birth.t |>  EcotoxSystems.robustmin
    end
...
"""
function estimate_emb_dev_time(p::ComponentVector; kwargs...)
...
robustmax(x::AbstractVector) = length(x)>0 ? minimum(x) : Inf

robuststd(x::AbstractVector) = x[isfinite.(x)] |> x -> length(x)>0 ? std(x) : error("No finite values found in x")
...
Infer post-metamorphic relative weight change from simulation output. 

`dt` is the time in days after metamorphosis at which the second weight measurement is taken. 
"""
function post_metamorphic_weightchange(sim::AbstractDataFrame; dt = 10)

    t1 = metamorphosis_timing(sim)[2]
    t2 = t1+dt
